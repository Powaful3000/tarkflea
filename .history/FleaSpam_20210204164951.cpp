...
void fastScreenshot() {
    HDC hwindowDC = GetWindowDC(tarkHANDLE);
    HDC hwindowCompatibleDC=CreateCompatibleDC(hwindowDC);


    HBITMAP dataBitMap = CreateBitmap();
    // create a bitmap
    HBITMAP hbwindow = CreateCompatibleBitmap(hwindowDC., tarkSize[0], tarkSize[1]);
...
void generateRandomDuration() {
    sleepDur = (( (float) rand() / RAND_MAX) / 2000.0f);
}
