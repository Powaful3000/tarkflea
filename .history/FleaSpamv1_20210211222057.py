...
def locateImage(file_loc, nickname, im, acc=0.9, callback=None):
...
class brrScreenshot:
    ...
    def go(self):
        while True:
            self.im = fastScreenshot(tarkHANDLE, 1920, 1080)
...
mySS = brrScreenshot()
screenshotThread = threading.Thread(name="Screenshots", target=mySS.go)
