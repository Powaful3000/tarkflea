...
def surch(file_loc, nickname, acc=0.85,callback=None, argsNum = 0):
    rawPos = imagesearch(file_loc, acc)
    if (rawPos[0]!=-1):
...
while(True):
    ...
    if ScriptEnabled:
        for h in range(10):
            randDur()
            sendKey(win32con.VK_F5,sleepDur)
            surch("./search/clockimage.png", "offer", 0.95, spamClickY, 1)
