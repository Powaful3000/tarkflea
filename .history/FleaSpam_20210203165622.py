...
const leftMonitorsOffset:int = 2560 #IMPORTANT (0 if only one monitor, I have a 21:9 2560x1080 monitor so I set to 2560.  Thank imagesearch for being trash.)
DownMonitorsOffset:int = 0 #IMPORTANT (same shit as before)
monitorSizeX:int = 1920
monitorSizeY:int = 1080
gameBorderH:int = 16
gameBorderV:int = 39
...
def surch(file_loc, nickname, acc=0.85,callback=None, location=None):
    ...
    if location == None:
        ...
        #rawPos = imagesearcharea(file_loc,int(round(location[0]+tarkPos[0])/2),int(round(location[1]+tarkPos[1])/2),int(round(location[0]+tarkPos[0])*2),int(round(location[1]+tarkPos[1])*2), acc)
        rawPos = imagesearcharea(file_loc,tarkPos[0],tarkPos[1],, acc)
    after = time.time()
