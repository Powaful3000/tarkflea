...
def fast_screenshot(hwnd, width, height):
...
def click_client_to(HANDLE, x:int, y:int):
...
def surch(file_loc, nickname, acc=0.9,callback=None):
    ...
    countSurch+=1
    before = time.time()
    img = fast_screenshot(tarkHANDLE,tarkSize[0], tarkSize[1])
...
def spamClickY():
    for i in range(5):
        for J in range(5):
            click_client_to(tarkHANDLE, posOffer[0], posOffer[1])
            sendKey(0x59,sleepDur)
        sendKey(win32con.VK_F5,sleepDur)
def clickFail():
    click_client_to(tarkHANDLE, posOK[0], posOK[1])
    sendKey(win32con.VK_F5,sleepDur)
