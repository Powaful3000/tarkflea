...
def imagesearcharea(image_object, x1, y1, x2, y2, precision=0.8, img_rgb=None):
    if im is None:
    ...
    res = cv2.matchTemplate(img_gray, image_object, cv2.TM_CCOEFF_NORMED)
...
def fastScreenshot2():
    ...
    return PIL.Image.frombytes("RGB", sct_img.size, sct_img.bgra, "raw", "BGRX").tobytes()
...
def locateImage(image_object, nickname, acc=0.9, callback=None):
    global surchTime, countSurch
    countSurch += 1
    before = time.time()
    #img = fastScreenshot(tarkHANDLE, tarkSize[0], tarkSize[1])
    img = fastScreenshot2()
    rawPos = imagesearcharea(image_object, 0, 0, 1920, 1080, acc, img)
    after = time.time()
...
def main():
    global tarkHANDLE
    clockImage = cv2.imread("./search/clockimage.png", 0)
    OKImage = cv2.imread("./search/NotFound.png", 0)
    BOTImage = cv2.imread("./search/BOT.png", 0)
    ...
    while(True):
        ...
        if ScriptEnabled:
            for _ in range(10):
                ...
                pressKey(win32con.VK_F5, sleepDur)
                locateImage(clockImage,
                            "offer", 0.85, spamClickY)
                locateImage(OKImage, "fail", 0.9, clickFail)
            locateImage(BOTImage, "BOT", 0.8, exit)
        else:
