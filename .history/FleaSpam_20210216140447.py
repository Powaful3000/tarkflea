...
def locateImages(machine: ScreenshotMachine, file_loc: tuple, nickname: tuple, acc: tuple = (0.9), callback: tuple = None) -> bool:
...
def main():
    ...
    while(True):
        ...
        if ScriptEnabled:
            ...
            clickF5()
            for _ in range(10):
                if (locateImages(machine, ("./search/clockImage.png", "./search/NotFound.png", "./search/BOT.png"),
                                 ("fail", "offer", "BOT"), (0.98, 0.8, 0.8), (clickFail, spamClickY, foundBot))):
                    break
            sleep(LOOPSLEEPDUR)
        else:
            if Now is None:
