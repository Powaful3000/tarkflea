...
class ScreenshotMachine:
    ...
    def __init__(self):
        ...
        proc1 = mp.Process(target=self.grabLoop, args=(self.childConn,))
        proc1.start()
        self.listener(self.parentConn)

    def listener(self, pipe):
        while True:
            if (pipe.poll()):
                self.latestImg = pipe.recv()

    def getLatest(self):
        img = self.parentConn.recv()
