...
int main()
{
    ...
    cout << MoveWindow(tarkHANDLE, tarkPos[0], tarkPos[1], tarkSize[0], tarkSize[1], FALSE);
    cout << SetActiveWindow(tarkHANDLE);
    // INPUT input[1];
    ...
    for (int i = 0; i<10;i++) {
        LRESULT sent = SendMessage(tarkHANDLE, WM_MOUSEMOVE, 0, MAKELPARAM(posOK[0],posOK[1]));
        cout << sent;
        sent = SendMessage(tarkHANDLE, BM_CLICK, 0, MAKELPARAM(posOK[0],posOK[1]));
        cout << sent;
    }
}