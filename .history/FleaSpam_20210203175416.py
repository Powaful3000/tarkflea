...
def locateImages(file_loc, nickname, acc=0.9,callback=None):
    ...
    while(True):
        ...
        if ScriptEnabled:
            for h in range(10):
                ...
                pressKey(win32con.VK_F5,sleepDur)
                locateImages("./search/clockimage.png", "offer", 0.85, spamClickY)
                locateImages("./search/NotFound.png", "fail", 0.9, clickFail)
            locateImages("./search/BOT.png","BOT", 0.8, exit)
        else:
            print("Paused")
            time.sleep(0.25)
