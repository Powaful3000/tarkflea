...
def locateImages(file_loc: tuple, nickname: tuple, acc: tuple = (0.9), callback: tuple = None):
    global surchTime, countSurch
    countSurch += 1
    before = time()
    img = fastScreenshot(tarkHANDLE, tarkSize[0], tarkSize[1])
    for i in range(len(file_loc)):
        rawPos = imagesearcharea(file_loc[i], 0, 0, 1920, 1080, acc[i], img[i])
        after = time()
        surchTime += (after-before)
        avg = computeAvgScans()
        if (rawPos[0] != -1):
...
def main():
    ...
    while(True):
        ...
        if ScriptEnabled:
            ...
            clickF5()
            for _ in range(3):
                locateImages(("./search/clockImage.png", "./search/NotFound.png", "./search/BOT.png"),
                             ("offer", "fail", "BOT"), 0.85, spamClickY)
                locateImage(, , 0.9, clickFail)
                locateImage(, , 0.8, foundBot)
            sleep(LOOPSLEEPDUR)
        else:
            if Now is None:
                Now = time()
