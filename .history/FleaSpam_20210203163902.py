...
gameBorderH:int = 16
gameBorderV:int = 39
areaOffer = (900, 50, 500, 1100)(500, 300, 1100, 900)
areaOK = (900, 50, 500, 1100)
...
def surch(file_loc, nickname, acc=0.85,callback=None, location=None):
    ...
    if location == None:
        ...
        rawPos = imagesearcharea(file_loc,location[0]+tarkPos[0],location[1]+tarkPos[1],location[2]+tarkPos[0],location[3]+tarkPos[1], acc)
    after = time.time()
    ...
    while(True):
        ...
        if ScriptEnabled:
            for h in range(10):
                randDur()
                sendKey(win32con.VK_F5,sleepDur)
                surch("./search/clockimage.png", "offer", 0.9, spamClickY, areaOffer)
                surch("./search/NotFound.png", "fail", 0.95, clickFail, areaOK)
            surch("./search/BOT.png","BOT", 0.8, exit, areaOK)
        else:
            print("Paused")
            time.sleep(0.25)
