...
def surch(file_loc, nickname, acc=0.85,callback=None, location=None):
    ...
    if location == None:
        ...
        rawPos = imagesearcharea(file_loc,int(round(location[0]+tarkPos[0])/2),int(round(location[1]+tarkPos[1])/2),int(round(location[0]+tarkPos[0])*2,(location[1]+tarkPos[1])*2, acc)
    after = time.time()
