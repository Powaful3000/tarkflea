...
clockTemplate = template = cv2.imread("./search/clockimage.png", 0)
nfTemplate = template = cv2.imread("./search/NotFound.png", 0)
botTemplate = template = cv2.imread("./search/BOT.png", 0)
...
def locateImage(template, nickname, acc=0.9, callback=None):
    global surchTime, countSurch
    countSurch += 1
    before = time()
    img = fastScreenshot(tarkHANDLE, tarkSize[0], tarkSize[1])
    rawPos = imagesearcharea(template, acc, img)
    after = time()
    ...
    while(True):
        ...
        if ScriptEnabled:
            for _ in range(10):
                ...
                pressKey(win32con.VK_F5, sleepDur)
                locateImage(clockTemplate,
                            "offer", 0.85, spamClickY)
                locateImage(nfTemplate, "fail", 0.9, clickFail)
            locateImage(botTemplate, "BOT", 0.8, exit)
        else:
            print("Paused", time())
            sleep(0.25)
